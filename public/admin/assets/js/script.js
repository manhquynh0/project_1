...
if (filterPrice) {
  ...
  if (valueCurrentMin && valueCurrentMax) {
    filterPrice.value = `${priceMin}-${priceMax}`;
  }
...
//End Filter Price
//Filter CategoryTree
const categoryTree = document.querySelector("[filter-categoryTree]")
if (categoryTree) {
  const url = new URL(window.location.href)
  categoryTree.addEventListener("change", () => {
    const value = categoryTree.value
    if (value) {
  ...
  if (valueCurrent) {
    categoryTree.value = valueCurrent;
  }
}
// End Filter CategoryTree
